...
For example, a 4th-order tensor with number of elements $[n_1, n_2, n_3, n_4] = [100, 100, 100, 100]$ when unfolded will have dimensions $= 100 \times 10^6$
...
The current implementation (matlab code provided) subsamples in the row space to then recover the right eigenvalue matrix with the low cost procedure described before. This is not optimal for a couple of reasons:
 - rank drop due to subsampling
 - computational cost increasing by using huge matrices to recover small matrices

 Both these reasons will be seen in the following discussion with pictures for the case of a three dimesnional tensor. The logic extends to higher dimensional tensors and the two factors get worse the higher the order of the tensor (the unfolded matrices are quite big).

 ### Visualization of the code provided
...
5. **Sign Correction**: Extract and apply sign corrections:
   ...
   $$V_{\text{red}} \leftarrow V_{\text{red}}^T \cdot \text{diag}(\text{signs})$$
    ...
    All in all steps 3 through 6 are technicalities and do not change the core idea of what the algorithm does.
...
![alt text](FULL_EXPLANATION/matrices_product.png)
